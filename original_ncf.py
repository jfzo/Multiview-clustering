...
class NCF(object):
    ...
    def run(self):
        ...
        # ranking
        ave_complexity = [np.mean([compute_solutions_complexity(self.Pi[j], rnd_p[i], K[j], kmax)[2] for i in range(N_rnd)]) for j in range(m)]
        ...
        while( not np.isinf(np.min(Aff)) ):
            ...
            new_complexity = np.mean([compute_solutions_complexity(self.Pi[newclustering_index], rnd_p[i], K[newclustering_index], kmax)[2] for i in range(N_rnd)])
